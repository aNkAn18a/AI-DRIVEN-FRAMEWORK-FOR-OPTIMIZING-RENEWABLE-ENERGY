...
1. Open the `.ino` firmware file located in the `firmware/` folder using Arduino IDE.
2. Replace the Wi-Fi credentials placeholders with your actual network name and password:
```cpp
...
3. *(Optional)* Update your Blynk Template ID and Auth Token if you are using your own Blynk Cloud project.
4. Select your **ESP32 Dev Module** board and the correct COM port, then click **Upload**.
5. During startup, the OLED will display `"CALIBRATING LDRs..."`. Shine equal light on both sensors for 3 seconds to auto-calibrate the system offset.
